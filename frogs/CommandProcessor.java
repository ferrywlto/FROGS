...
public class CommandProcessor
{
	...
	{	
		if(validateCommand(input))
		{
			try
			{
				...
				if(cmdType.equals("loginRequest") && (connection.getState() == ConnectionState.INIT))
				{
					...
					Logger.printMsg(connection,"Request to login with username:"+username+" password:"+password+".");
					processLogin(username,password);
				}
				else if(cmdType.equals("listChessRequest") && (connection.getState() == ConnectionState.LOGGED))
				{
					Logger.printMsg(connection,"Request to list created chesses.");
					processListChess();
				}
				else if(cmdType.equals("createGameRequest") && (connection.getState() == ConnectionState.LOGGED))
				{
					...
					Logger.printMsg(connection,"Request to create chess game:"+chessName+".");					
				...
				else if(cmdType.equals("joinGameRequest") && (connection.getState() == ConnectionState.LOGGED))
				{
					...
					Logger.printMsg(connection,"Request to join game "+gID+".");
					processJoinGame(gID);
				}
				...
				{
					Logger.printMsg(connection,"Request to quit current game.");
					processQuitGame();
				}			
				else if(cmdType.equals("listGameRequest") && (connection.getState() == ConnectionState.LOGGED))
				{
					Logger.printMsg(connection,"Request to list available games.");
					processListGame();
				}
				else if(cmdType.equals("gameCommand") && (connection.getState() == ConnectionState.PLAY_CMD))
				{
					...
					Logger.printMsg(connection,"Request to process game command. type:"+type+" param:"+param);					
				...
				{
					Logger.printMsg(connection,"Request to disconnect.");
					processDisconnect();
				}
				else
				{
			...
			catch (NullPointerException e)
			{
				...
				//e.printStackTrace();
				Logger.printMsg(connection,"Suspicious action detected: Null string paramater.");
				connection.sendMessage(Logger.errorMsg("Internal Server Error. Connection Terminated. (E002)"));
		...
		else
		{
			...
			Logger.printMsg(connection,"Invalid command detected.");
	...
	protected void processLogin(String username, String password)
	{
		...
		{					
			...
			Logger.printMsg(connection,"Login success.");
			connection.sendMessage(Logger.responseMsg("login_result","true"));
		}
		else
		{
			Logger.printMsg(connection,"Login Failed.");
			connection.sendMessage(Logger.responseMsg("login_result","false"));
		}
	}
	...
	protected void processListChess()
	{
		...
		connection.sendMessage(Logger.responseMsg("listChess_result",result));
	}
	...
	protected void processCreateGame(String chessName, String gameName, String gameDesc)
	{
		...
		if(fileName == null)
		{
			Logger.printMsg(connection,"No such chess definition created by '"+connection.getUsername()+"' called '"+chessName+"'.");
			connection.sendMessage(Logger.responseMsg("create_result","<status>false</status><spec/>"));					
			return;
		}
		...
		if(gID > 0 && gameManager.getGame(gID).join(connection.getConnID()))
		{
			...
			connection.sendMessage(Logger.responseMsg("create_result",result));
		}
		else if(gID == 0)
		{
			Logger.printMsg(connection,"Problem occur in parsing chess. Game createion failed.");
			connection.sendMessage(Logger.responseMsg("create_result","<status>false</status><spec/>"));
		}
		else if(gID == -1)
		{
			Logger.printMsg(connection,"Game name already exists. Game creation failed.");
			connection.sendMessage(Logger.responseMsg("create_result","<status>false</status><spec/>"));					
		}
		else if(gID == -2)
		{
			Logger.printMsg(connection,"File not found? Game creation failed.");
			connection.sendMessage(Logger.responseMsg("create_result","<status>false</status><spec/>"));					
		}
		else if(gID == -3)
		{
			Logger.printMsg(connection,"File cannot read? Game creation failed.");
			connection.sendMessage(Logger.responseMsg("create_result","<status>false</status><spec/>"));					
		}	
	}
	...
	{
		Channel game = gameManager.getGame(gID);
		...
		if(game != null)
		{
			...
			{
				Logger.printMsg(connection, "Failed to join game "+game.getName());
				connection.sendMessage(Logger.responseMsg("join_result","<status>false</status><spec/>"));
			}
		}
		else
		{
			Logger.printMsg(connection, "No such game.");
			connection.sendMessage(Logger.responseMsg("join_result","<status>false</status><spec/>"));
		}	
	}
	...
	protected void processQuitGame()
	{
		...
		if(quitSucc)
		{
			connection.sendMessage(Logger.responseMsg("quit_result","<status>true</status>"));
		}
		else
		{
			Logger.printMsg(connection,"Failed to quit game?");
			connection.sendMessage(Logger.responseMsg("quit_result","<status>false</status>"));
		}
	}
	...
	protected void processListGame()
	{
		...
		{
			int i = e.nextElement();
			Channel game = gameManager.getGame(i);
			String tmpStr = "";
		...
		connection.sendMessage(Logger.responseMsg("listGame_result",resultStr));		
	}
	...
	protected void processDisconnect()
	{
		...
		{
			Channel game = gameManager.getGame(connection.getGameID());
		...
		connection.sendMessage(Logger.responseMsg("disconnect_result","true"));
