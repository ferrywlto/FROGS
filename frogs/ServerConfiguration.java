...
{
	protected static Hashtable<String, String> parameters;
		...
		try
		{
			if(!loaded)
			{
				...
				if(detector.exists())
				{
					...
					Logger.printMsg("ServerConfiguration","Load Server Configuration File: "+path);
					Document configDoc = XMLLoader.getXMLDocumentFromFile(path);
			
					Element root = configDoc.getDocumentElement();
					NodeList nodes = root.getChildNodes();
					Logger.printMsg("ServerConfiguration","Parsing Server Config Parameters...");
					...
					listParameters();
					Logger.printMsg("ServerConfiguration","Server Configuration Loading Completed.\n");
					loaded = true;
					return true;
				}
				else
	...
	public static void listParameters()
	{
		...
		while(e.hasMoreElements())
		{
			...
			Logger.printMsg("ServerConfiguration",id+" = "+parameters.get(id));
