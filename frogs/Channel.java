...
public class Channel 
{
	...
	public Channel(int gID, String gName, String gDesc, String creator, Chess chess)
	{
		this.gameID = gID;
		this.gameName = gName;
		this.gameDesc = gDesc;
	...
	public void processCommand(int connID, String type, String param)
	{
			...
			this.currentPlayerConnection.getState() == ConnectionState.PLAY_CMD)
		{
			...
			if(pID == null) {
				pConn.sendMessage(Logger.errorMsg("No such player?"));
				return;
			}
			...
			if(rType == RuleType.PREMOVE)
			{
				...
				if(obj == null || player == null) {
					pConn.sendMessage(Logger.errorMsg("Invalid Object/Player."));
					return;
				}
				
				// Impossible that obj not belongs to player
				if(player != obj.getOwner()){ 
					pConn.sendMessage(Logger.errorMsg("You are not object owner."));
					return;
				}
				...
				pConn.sendMessage(Logger.responseMsg("gameCommand_result", resultStr));
			}
			else if(rType == RuleType.POSTMOVE)
			{
				...
				{	
					...
					String output = Logger.responseMsg("game_event", resultStr);
					connManager.multicastMessage(this.getConnectionIDList(), output);
					endTurn();
				}
				else
				{
					pConn.sendMessage(Logger.errorMsg("Tile not moveable."));
					return;
				}
			}
			else
			{
				...
				pConn.sendMessage(Logger.errorMsg("Invalid Command Type."));
				return;
			}
		}
		else
		{
			pConn.sendMessage(Logger.errorMsg("Please wait while command processing."));
			return;
		}
	}
