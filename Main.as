...
_global.xmlSock.onConnect = function(success:Boolean) 
{
	if(success)
	{
		_root.connect_btn.enabled = false;
		_global.showMsg("Successfully connected to server.");
	}
	else
	{
		_global.showMsg("Failed to connect.");
	}
};
_global.xmlSock.onClose = function()
{
	...
	_global.showMsg("Connection closed.");
};
	...
	if(newXML.firstChild.nodeName == "wbcgdp_response")
	{
		...
		if(type_str.toString() == "login_result")
		{
			if(resultNode.firstChild.toString()=="true")
			{
				...
				_global.chessGame.showMsg("Login success.");
			}
			else
				_global.chessGame.showMsg("Failed to login.");
		}
		else if(type_str.toString() == "listChess_result")
		{
			if(resultNode.childNodes.length == 0)
				_global.chessGame.showMsg("You have not designed any chess game.");
			else
			{
				_global.chessGame.showMsg("Chess games designed:");
		...
		{
			if(resultNode.childNodes.length == 0)
				_global.chessGame.showMsg("No games available.");
			else
			{
				_global.chessGame.showMsg("Available games:");
				for(var i=0;  i<resultNode.childNodes.length; i++)
				{
					...
					_global.chessGame.showMsg("Game Name: "+gName+" ("+gID+")\tCreator: "+gCreator+"\tDescription: "+gDesc);
				}
			}			
		}
		else if(type_str == "create_result")
		{
				...
				for(var i=2; i<resultNode.childNodes.length; i++)
				{
					...
					if(tempNode.nodeName == "player")
					{
						...
						if(nickName == _global.username)
						{
							...
							_global.chessGame.showMsg(nickName+" has joined the game.");
						}
					}
				}
				_global.joined = true;
			...
			{
				_global.chessGame.showMsg("Failed to create game.");
			}
		}
		else if(type_str == "join_result")
		{
			...
			{
				_global.chessGame.showMsg("Failed to join game.");
			}		
		}
		else if(type_str == "game_event")
		{
			...
			if(_global.joined == true)
			{
				if(event_type == "join")
				{
					...
					_global.chessGame.showMsg(event_msg);
				}
				...
				else if(event_type == "action")
				{
					for(var i=0; i<event_typeNode.childNodes.length; i++)
					{
						...
						if(actionNode.nodeName == "move")
						{
							...
							_global.chessGame.showMsg("Object "+objID+" has moved to location "+tileID+".");
						}
						else if(actionNode.nodeName == "remove")
						{
							...
							_global.chessGame.showMsg("Object "+objID+" has removed.");
						}
					}
				}
		...
		else if(type_str == "error")
		{
			...
			_global.chessGame.showMsg("Error encountered: "+msg.toString());
